...
#include <TLatex.h>
#include <TLine.h>
void uU_mM_Xsec_Pcm(void)
{
 TCanvas *Can = new TCanvas("c","u,U ->m,M",200,10,600,400);
 double xMin=1.000000E+02, xMax=6.500000E+03, yMin=8.536082E-07, yMax=5.345862E-01;
 char buff[2000];
 ...
 hr->SetTitle("#sigma_{u,U #rightarrow m,M} x P_{CM}");
 hr->SetXTitle("Pcm");
 hr->SetYTitle("v*sigma[pb]");
 double  Y0[100];
 FILE*f=fopen("uU_mM_Xsec_Pcm.tab","r");
 for(int i=0;i<100;)
 {
  fscanf(f,"%[^\n]%*c",buff);
