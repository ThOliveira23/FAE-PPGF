...
#include <TLatex.h>
#include <TLine.h>
void bB_mM_Xsec_Pcm(void)
{
 TCanvas *Can = new TCanvas("c","b,B ->m,M",200,10,600,400);
 double xMin=1.000000E+02, xMax=6.500000E+03, yMin=2.121242E-06, yMax=1.335532E+00;
 char buff[2000];
 ...
 hr->SetTitle("#sigma_{b,B #rightarrow m,M} x P_{CM}");
 double  Y0[100];
 FILE*f=fopen("bB_mM_Xsec_Pcm","r");
 for(int i=0;i<100;)
 {
  fscanf(f,"%[^\n]%*c",buff);
